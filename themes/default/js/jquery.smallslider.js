 ...
 */ (function ($) {
    ...
    $.smallslider = function (elm, options) {
        ...
        _this.createButtons = function () {
            ...
            buttons = '<div class="smallslider-btns" style="margin-right:50%; bottom: 15px;">' + buttons + '</div>';
            var left = 0,
                right = 0,
                top = 0,
                bottom = 0;
        ...
        _this.createText = function () {
            ...
            $('<h3 class="smallslider-tex"></h3>').css(style_tex).html("").appendTo(_this.$elm);
    ...
    $.smallslider.defaults = {
        ...
        buttonOffsetY: 15, // 竖直方向上的按钮偏移位置，指向中心部移动多少，这里是数值，不加px
        buttonSpace: 4, // 按钮之间的间隔 单位为像素，但不要加px
        showText: false, // 是否显示标题，如果不显示，则只显示按钮
        showButtons: true, // 是否显示按钮，默认显示
        textLink: false, // 是否给显示的标题加上链接，如果为false，只显示标题，标题不可单击
