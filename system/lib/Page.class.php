 ...
 * @return string
 ...
 {
	$plus=ceil(intval($this->pageBarNum)/2);
	
	if(intval($this->pageBarNum)-$plus+$this->nowIndex>$this->totalPage)
		$plus=(intval($this->pageBarNum)-$this->totalPage+$this->nowIndex);
		
	$begin=$this->nowIndex-$plus+1;
	$begin=($begin>=1)?$begin:1;
	$return='';
	for($i=$begin;$i<$begin+intval($this->pageBarNum);$i++)
	{
		if($i<=$this->totalPage)
		{
			if($i!=$this->nowIndex)
